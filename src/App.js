...
class App extends Component {
  constructor() {
    ...
    this.handleAddSpotifyPlaylist = this.handleAddSpotifyPlaylist.bind(this);
  }
  ...
  componentDidMount() {
    ...
    console.log('Componente montato', queryObject);
  }

  handleSpotifySearch(valueSpotifySearch) {
    console.log(valueSpotifySearch, this.state.isLoggedInState);
    if (!this.state.isLoggedInState) {
      ...
      const accessToken = querystring.parse(window.location.hash.replace('#', '')).access_token;
      console.log(accessToken);
      fetch('https://api.spotify.com/v1/search?q=royals&type=track,album,artist', {
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json',
          'Authorization': 'Bearer ' + accessToken
  ...
  handleAddSpotifyPlaylist(newPlaylistName) {
